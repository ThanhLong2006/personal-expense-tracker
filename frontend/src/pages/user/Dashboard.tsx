...
const Dashboard = () => {
  ...
  const ExpenseTrendChart = ({
    ...
    const selectedDayLabel = (() => {
      ...
      return isNaN(dateObj.getTime()) ? null : format(dateObj, "yyyy-MM-dd");
    })();

    if (loading)
      return (
        <div className="h-[280px] w-full flex items-center justify-center bg-slate-50/50 rounded-[2rem] animate-pulse">
          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#00C4B4]"></div>
        </div>
      );
          ...
          >
            ...
            <XAxis
              ...
              tickFormatter={(str) => {
                ...
                return isNaN(date.getTime()) ? str : format(date, "dd/MM");
              }}
            ...
            <Tooltip
              ...
              formatter={(value: number, name: string) => {
                const isThu = name === "thu";
                const label = isThu ? "Tiền thu" : "Tiền chi";
                const color = isThu ? "#00C4B4" : "#ef4444";

                return [
                  <span key="val" style={{ color: color, fontWeight: "800" }}>
            ...
            {showReference && selectedDayLabel && (
              <ReferenceLine
                x={selectedDayLabel}
                stroke="#6366f1"
                strokeWidth={2}
